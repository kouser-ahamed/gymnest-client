...
const ClassDetailsClient = ({ classDetails, user: serverUser }) => {
  ...
  const user = {
    ...(sessionUser || {}),
    ...(serverUser || {}),
    id: serverUser?.id || serverUser?._id || sessionUser?.id,
    _id: serverUser?._id || serverUser?.id || sessionUser?._id,
    name: serverUser?.name || sessionUser?.name,
    email: serverUser?.email || sessionUser?.email,
    image: serverUser?.image || sessionUser?.image,
    role: serverUser?.role || sessionUser?.role || "member",
    status: serverUser?.status || sessionUser?.status || "active",
  };
  ...
  const isRestrictedUser = Boolean(user?.id) && userStatus !== "active";

  const restrictedMessage = "You restricted by admin";
